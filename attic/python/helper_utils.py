...
def plot_waveform(
    ...
    if plot_eddy:
        ...
        for lam in all_lam:
            lam = lam * 1.0e-3
            r = np.diff(np.exp(-np.arange(G[0].size + 1) * dt / lam))[
                ::-1
            ]  # TODO: 3-axis case, right now just assumes 1 axis
    ...
    G_ss = np.concatenate(
        (
            np.linspace(
                params["g_ss"] * 1e-3,
                params["g_ss"] * 1e-3,
                int(ceil(params["p_ss"] * 1e-3 / params["dt"])),
            ),
        ...
        / (
            ...
            ** 3
            ...
            - (
                ...
                * h**2
                - 6 * GAM**2 * params["T_90"] * params["T_180"] * h**2 * zeta
                - 6 * GAM**2 * params["T_90"] * params["T_readout"] * h**2 * zeta
                + 6 * GAM**2 * params["T_180"] * params["T_readout"] * h**2 * zeta
            )
            / (4 * GAM**2 * h**2)
            + (
                zeta**2
